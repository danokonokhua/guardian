...
/** Requested resource already exists / conflicting state. */
export class ConflictError extends AppError {
  constructor(message = "The request conflicts with existing state.", details?: unknown) {
    super({ code: "CONFLICT", status: 409, message, details });
  }
}

/** Client exceeded an allowed request rate. */
export class RateLimitError extends AppError {
  constructor(message = "Too many requests. Please retry later.", details?: unknown) {
