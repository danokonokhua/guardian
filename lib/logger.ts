...
const SENSITIVE_KEY_PATTERN =
  /password|passphrase|secret|token|api[-_]?key|authorization|credential|cookie|session/i;
...
/**
 * Shared sensitive-value redaction (Phase 1B-08): the same policy the logger
 * applies to its own context, exported for the observability layer (Sentry
 * event hints) so nowhere else needs to invent its own sanitizer.
 */
