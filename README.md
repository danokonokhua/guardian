...
> **Current phase: 1B-01 — Project Foundation.**
> This repository currently contains the application foundation only (framework,
> configuration layer, error handling, logging, testing, health endpoint). Monitoring,
> authentication, database, dashboard, and AI modules arrive in later phases under the
> approved Phase 1A architecture (`docs/`).
...
| **1B-01 — Repository & application foundation**                                 | **Complete — awaiting human review** |
| 1B-02+ — Config/testing/CI hardening, database, auth, tenancy, jobs, monitoring | Not started                          |
...
- **Node.js ≥ 20.9** (LTS). `.nvmrc` pins major `20`; Node 22 LTS also works.
...
`DATABASE_URL`, `SUPABASE_*`, `RESEND_API_KEY`, AI/payment keys are **reserved for later
phases** and documented in `.env.example` — they are not read yet. All `.env*` files are
