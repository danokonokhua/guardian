...
describe("prisma schema validity", () => {
  it("validates cleanly (offline, dummy envs)", () => {
    const output = execSync(
      'DATABASE_URL="postgresql://u:p@localhost:5432/guardian" DIRECT_URL="postgresql://u:p@localhost:5432/guardian" npx prisma validate --schema db/schema.prisma',
...
describe("tenant isolation ownership", () => {
  it("every tenant-owned model carries a required organizationId", () => {
    for (const model of ["OrganizationMember", "Business", "Website", "Monitor", "Issue"]) {
