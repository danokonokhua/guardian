...
describe("MonitoringPanel SEO option", () => {
  it("creates an SEO monitor through the tenant-scoped monitor endpoint", async () => {
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      const url = String(input);
      if (url.endsWith("/monitors") && init?.method === "POST") {
        return new Response(JSON.stringify({ data: { id: "monitor-seo" } }), { status: 201 });
      }
    ...
    await user.selectOptions(screen.getByLabelText("Check type"), "SEO");
