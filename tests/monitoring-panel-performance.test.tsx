...
describe("MonitoringPanel Performance option", () => {
  it("creates a Performance monitor through the tenant-scoped endpoint", async () => {
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      const url = String(input);
      if (url.endsWith("/monitors") && init?.method === "POST") {
        return new Response(JSON.stringify({ data: { id: "monitor-performance" } }), {
          status: 201,
        });
      }
    ...
    await user.selectOptions(screen.getByLabelText("Check type"), "PERFORMANCE");
