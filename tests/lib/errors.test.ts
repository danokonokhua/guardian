...
describe("error subclasses", () => {
  ...
  it("ConflictError maps to 409 / CONFLICT", () => {
    const error = new ConflictError("Website already exists", { hostname: "example.com" });
    expect(error.code).toBe("CONFLICT");
    expect(error.status).toBe(409);
    expect(error.message).toBe("Website already exists");
    expect(error.details).toEqual({ hostname: "example.com" });
  });

  it("RateLimitError maps to 429 / RATE_LIMITED", () => {
    const error = new RateLimitError();
    expect(error.code).toBe("RATE_LIMITED");
    expect(error.status).toBe(429);
    expect(error.message).toBe("Too many requests. Please retry later.");
  });
